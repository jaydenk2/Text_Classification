...
class CNNBlock(nn.Module): # this is almost the exact same from lecture 22
    def __init__(self, in_channels, out_channels, kernel_size):
        super().__init__()
        padding = int((kernel_size-1)/2) # to maintain the same length after conv
        self.block = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride= 2) # 2x2 pooling
        )
    def forward(self, x):
        return self.block(x)
